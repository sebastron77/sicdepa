...
if (isset($_POST['add_rem_mens'])) {
    ...
    if (empty($errors)) {
        ...
        $sql = "UPDATE rel_detalle_renum SET renum_mens='{$renum_mens}', nombre_act_indus='{$nombre_act_indus}', act_indus='{$act_indus}', 
                nombre_act_fin='{$nombre_act_fin}', act_finan='{$act_finan}', tipo_serv_prof='{$tipo_serv_prof}', serv_prof='{$serv_prof}', 
                otros_info='{$otros_info}', otros='{$otros}', subtotal2='{$subtotal2}', subtotal1_2='{$subtotal1_2}', cony_deduce_imp='{$cony_deduce_imp}', 
                ingr_cony='{$ingr_cony}', suma_ab='{$suma_ab}'
        ...
        if ($result && $db->affected_rows() === 1) {
            ...
            redirect('edit_rem_mens.php?id=' . (int)$detalles['id_rel_detalle_'], false);
        } else {
            $session->msg('d', ' No se pudo agregar la remuneración del cargo que inicia.');
            redirect('edit_rem_mens.php', false);
        }
    } else {
